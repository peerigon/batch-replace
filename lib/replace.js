...
function runModules(str, modules) {
    ...
    modules.forEach(function forEachModule(module, index) {
        ...
        runModule(module, index, str, replacements);
    });
 ...
 * @private
 * @param {ReplacementModule} module
 * @param {Number} modulePriority
 * @param {String} str
 * @param {Array<Replacement>} replacements
 */
function runModule(module, modulePriority, str, replacements) {
...
function applyReplacements(str, replacements) {
    ...
    for (i = 0; i < replacements.length; i++) {
        newReplacement = replacements[i];

        if (newReplacement) {
            if (!replacement || newReplacement.priority > replacement.priority) {
                replacement = newReplacement;
                replacementEnd = i + replacement.oldStr.length - 1;
                result += tempResult;
            }
        }
